...
public class PersonalContactPanel extends JPanel {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PersonalContactPanel.class);
    ...
    public static void initializeTableData() {
        try {
            ResultSet resultSet = dbMySQL.executeStoredProcedureQuery("spGetAllViewPersonalContactDetail");
    ...
    private JPanel getTopPanel() {
        ...
        JLabel labelTitle = new JLabel("Personal Contacts");
    ...
    private void addListener() {
        ...
        buttonAddContact.addActionListener(e -> {
            NavBarPanel.SwitchToPersonalContactManagePanel();
        });
        ...
        buttonEditContact.addActionListener(e -> {
            if (tableContact.getSelectedRowCount() > 0) {
                ...
                NavBarPanel.SwitchToPersonalContactManagePanel();
                PersonalContactManagePanel.setContent(selectedContact);
            } else {
        ...
        buttonDeleteContact.addActionListener(e -> {
            if (tableContact.getSelectedRowCount() > 0) {
                ...
                if (answer == 0) {
                    try {
                        ResultSet resultSet = dbMySQL.executeStoredProcedureQuery("spGetPersonalAddressID", new String[]{contactID});
        ...
        buttonSearchContact.addActionListener(e -> {
            ...
            if (text.isEmpty()) {
                ...
                try {
                    ResultSet resultSet = dbMySQL.executeStoredProcedureQuery("spSearchPersonalContact", new String[]{text});
                    tableModel = Tools.resultSetToTableModel(resultSet);
                    tableContact.setModel(tableModel);
                } catch (SQLException ex) {
                    ex.printStackTrace();
